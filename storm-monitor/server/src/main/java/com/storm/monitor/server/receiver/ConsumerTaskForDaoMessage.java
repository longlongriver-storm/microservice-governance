 ...
 * </ul>
 */
public class ConsumerTaskForDaoMessage extends Thread {

    private static Logger LOG = LoggerFactory.getLogger(ConsumerTaskForDaoMessage.class);
    ...
    public ConsumerTaskForDaoMessage(BlockingQueue<MessageTree> daoMinuteStatQueue) {
        this.daoMinuteStatQueue = daoMinuteStatQueue;
    }

    @Override
    public void run() {
        DaoMonitorLogService daoMonitorLogService = null;
        List<DaoMonitorLog> currentSaveMsg = new ArrayList();
        ...
        while (true) {
            try {
                //从消息队列中获取一个消息
                MessageTree event = daoMinuteStatQueue.poll();

                if (event != null) {
                    DaoMonitorLog serviceMonitorLog = DaoMonitorLog.buildDaoMonitorLog(event);
                ...
                if (currentSaveMsg.size() >= 50
                    ...
                    if (daoMonitorLogService == null) {
                        daoMonitorLogService = ApplicationContextUtil.getBean(DaoMonitorLogService.class);
                    }
                    //批量保存一次日志数据
                    saveCount = daoMonitorLogService.addDaoMonitorLogBatch(currentSaveMsg);
                ...
                if (saveCount>0 && (allSaveCount % 1000L < 50L)) {
                    LOG.info("DaoMessageConsumerTask,all save success message count is {},"
                ...
                LOG.error("DaoMessageConsumerTask is error,{}", e);
            }
